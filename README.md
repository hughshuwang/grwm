...
- UNSOLVED: USB drive turned Read-only and cannot log in for unknown reasons `systemd-journald: Failed to write entry, ignoring, Read-only file system`. POSSIBLE FIX: use a live USB to run `sudo fdisk -l; sudo umount /dev/sdaX; sudo fsck -f /dev/sdaX` (first check disk scheme with fdisk -l, then umount the one with (corrupted) Linux File System, then fsck it and repair), [ref](https://askubuntu.com/questions/1135389/how-to-fix-read-only-file-system-on-18-04), [ref](https://askubuntu.com/questions/101637/usb-turn-write-protection-off), [ref](https://askubuntu.com/questions/753108/error-message-ext4-fs-error-ext4-find-entry-1451/768813#768813) (The filesystem goes read-only when file system errors are detected that cannot be fixed on the mounted filesystem. It's a security measure to prevent further data loss. This can be caused by either hardware issues (i.e. your disk is failing, go Mint menu > Disks, select your disk and check the SMART data for your drive from the hamburger menu at the top, in particular bad/relocated sectors), or a software issue (e.g. the old kernel version you're running has some known ext4 bugs, you could start by applying some updates to that system, those are being offered for a reason). **NOTE**: don't disconnect the drive while the device is working, avoid plugging in the drive when using Windows.
- Backlight setups, [ref](https://askubuntu.com/questions/715306/xbacklight-no-outputs-have-backlight-property-no-sys-class-backlight-folder)
- Google Chrome frozen when using i3 after updating to Pop OS 20.10, it could be the graphic driver issue, add `Option "DRI" "False"` to `/etc/X11/xorg.conf` [ref](https://unix.stackexchange.com/questions/524205/help-chromium-display-frozen-but-the-app-keeps-working). Log in to regular pop desktop, working fine, then back to i3, problem fixed.
